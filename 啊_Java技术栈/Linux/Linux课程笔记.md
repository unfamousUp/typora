...
- -l：`list`以列表形式查看
- -h：`human-readable`，需要配合`-l`使用，以更加人性化的方式显示文件大小
- -a：`all`，列出全部文件包含显示隐藏文件/文件夹
...
参数：目标目录，要切换去的地方，没有参数默认切换到`当前登录用户HOME目录`
...
- 参数：被创建文件夹的路径
- 选项：-p，可选，表示创建前置路径，适用于创建连续多层级的目录

> 注意：创建文件夹需要在HOME目录内，目录外操作设计到权限问题
...
cat：`concatenate(连接)`，显示文件内容或将多个文件连接（合并）成一个文件
...
-r：`recursive`，递归的，表示进行递归复制，复制目录及所有子目录和文件
...
- 参数：支持多个，每一个表示被删除的文件或文件路径，空格进行分隔
- 选项：-r，删除文件夹使用
...
- 路径，搜索的起始路径
- 参数，搜索的关键字，支持通配符*， 比如：`**test*`表示搜索任意以test开头和结尾的文件

语法2：`find 路径 -size +|- n[kMG]`
...
- 选项，-c，`characters`统计字符数量
- 选项，-m，`Megabyte`统计字节数量
- 选项，-l，`line`统计行数
- 选项，-w，`word`统计单词数量
